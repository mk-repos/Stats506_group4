...
```{python, python.reticulate=FALSE, eval=FALSE}
filename = "./maps_comp/cb_2018_us_state_500k.shp"
...
The SAS will store the data into `DATA` type. And we can treat it as usual `DATA` table. But fortunately, in sas, we don't need any further modification for the shapefile. The only things we need to do is to prepare our value data which will be discussed in following section.
...
```{python, python.reticulate=FALSE, eval=FALSE}
sf_us.plot()
plt.savefig('./python_files/whole_map_python.png')
plt.show()
```
...
plt.savefig('./python_files/dropped_map_python.png')
plt.show()
```
...
ax2.axis('off')
plt.savefig('./python_files/transformed_map_python.png')
plt.show()
```
...
plt.axis('off')
plt.savefig('./python_files/final_map_python.png')
plt.show()
```
...
We will use the gmap procedure in SAS to draw the choropleth map in SAS.
...
proc gmap
	...
	data = <Value Data>;
	id <id from map>; 
	choro <value from data>;
run;
```

SAS have done most of works for us. The only things we need to do to draw the choropleth map is to prepare the value data which will be discussed in next section. 
...
proc gmap
	...
	data = USmap all;
	id stusps;
	choro stusps/ nolengend;
run;
```

![***Figure:US cholopleth map***](SAS_document/uscholoplethmap.png)

***

**Example**: US - MIchigan

Of course, we can just show several states we want. The following code will show the outline of the Michigan state.
...
![***Figure:Michigan map***](SAS_document/Michigan.png)
...
![***Table:first 5 observations of Quarterly Data***](python_files/sf_merge.png)
...
![**shapefile first row**](SAS_document/shapefile_firstfewrow.png)
...
So, we will group our with three output variables: 
...
![***Table:first 10 observations of Value Data***](SAS_document/wide_case_data.png)
...
def draw_map_quarter(sf_obj, target_q,maxlim):
    ...
    plt.savefig("./python_files/{}".format(name))
    plt.show()
    return
```
...
![***Figure: Total count of COVID-19 positives in each state, first quarter of 2020***](./python_files/2020Q1.png)
...
![***Figure: Total count of COVID-19 positives in each state, second quarter of 2020***](./python_files/2020Q2.png)
...
![***Figure: Total count of COVID-19 positives in each state, third quarter of 2020***](./python_files/2020Q3.png)
...
![***Figure: Total count of COVID-19 positives in each state, fourth quarter of 2020***](./python_files/2020Q4.png)


### SAS {.tabset .tabset-pills}

Now, we have everything we need to draw choropleth map. And the results are shown below including code to access it. But as you can see from the below results, the colormap for each choropleth map is the same which is hard for us to compare across the quarters. In the extended example, we gonna show how to set up the color attributes for our choropleth map so we can compare across the quarter.
...
![***Figure: Total count of COVID-19 positives in each state, first quarter of 2020***](SAS_document/Q1.png)
...
![***Figure: Total count of COVID-19 positives in each state, second quarter of 2020***](SAS_document/Q2.png)
...
![***Figure: Total count of COVID-19 positives in each state, third quarter of 2020***](SAS_document/Q3.png)
...
![***Figure: Total count of COVID-19 positives in each state, fourth quarter of 2020***](SAS_document/Q4.png)
...
![***Table:first 5 observations of Monthly Data***](python_files/sf_merge_m.png)
...
# save as html  
fig.write_html('./python_files/Month_case.html', config=config)
```




```{r, echo=FALSE, cache=TRUE, fig.cap=cap}
cap = paste0("***Figure: Interactive choropleth map of total count of",
             " COVID-19 positives per month in each state***")
htmltools::includeHTML("./python_files/Month_case.html")
```

![***Figure: Interactive choropleth map of total count of COVID-19 positives per month in each state***](./python_files/Month_case.html)


# Extended Example: Colored Cholopleth map in SAS
