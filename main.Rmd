...
For shape file,we didn't need any further steps. We can directly use the shape file we just downloaded. But for the input data, we need to ensure the id in the input data will match any key variables in shape file. 

For example, since we want to show the number of positive cases for each state, thus we want the shape file to show under state level. There are two variables related with `state name` in shape file. One is STUSPS which is abbreviation of the state name and the other is 
<png shown print result of shape file>

Based on the data you have, you need to ensure any variable in your input data will match any key variables in shape file. We will discuss more in the next section. 
