...
There are not many things you have to worry about when drawing a map, but it should be noted that geographic coordinate systems are actually quite complex. It is difficult to describe the position on the Earth's surface because the Earth is not exactly spherical and cannot be transformed into Cartesian coordinates without some distortion.
...
***Geographic CRSs*** approximate the Earth's surface by an ellipsoid shape and identify a location on the surface by using longitude and latitude. The most common *Geographic CRSs* include:
...
***Projected CRSs*** refer to the coordinate systems used for 2-dimensional projection. When drawing a map, locations on the Earth's surface must be projected onto a flat paper, display, etc., which may result in some distortion. In practice, we need to project a point defined by *Geographic CRSs* onto 2-dimensional x (east-west) axis and y (north-south) axis. There are many different projection methods available, and each has both strengths and weaknesses. You should carefully choose one for its intended purpose. The most common *Projected CRSs* include:
...
draw_map_quarter = function(sf_obj, target_q, max_lim) {
  sf_obj %>% 
    ...
    # Apply diverging color scheme (blue < red)
